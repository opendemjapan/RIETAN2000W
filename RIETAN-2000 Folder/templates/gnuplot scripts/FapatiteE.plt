...
if (iout == 2) \
   ...
   set size 2.0, 1.25; \
   set output 'FapatiteE.eps'

xmin = 15 # Minimum for the x-axis
xmax = 125 # Maximum for the x-axis
ymin = -5000 # Minimum for the y-axis
ymax = 21000 # Maximum for the y-axis
set pointsize 0.7 # Size of '+' symbols for observed intensities
offset_delta = -0.11 * (ymax - ymin) # Offset of the difference curve along the y axis
len_bar = 0.011 * (ymax - ymin) # Length of bars for peak positions
set bar 0
y_phase1 = -0.03 * (ymax - ymin) # y coordinate of bars for phase No. 1
set ytics 0, 5000, 20000 # Start, increment, and end values of tics for the  y axis
...
if (iout == 0) plot [*:*] [*:*] \
   'FapatiteE.pat'using 1:3 notitle with lines linetype 11 linewidth 1, \
...
if (iout == 2) plot [xmin:xmax] [ymin:ymax] \
   'FapatiteE.pat' using 1:3 notitle with lines linetype 5 linewidth 1, \
